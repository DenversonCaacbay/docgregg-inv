<?php
    ...
    $view = $staffbmis->view_services_confinement($page, $recordsPerPage);
    $totalRecords = $staffbmis->count_services_confinement(); // get the total number of records

// Calculate the total number of pages
    $totalPages = ceil($totalRecords / $recordsPerPage);
    function getServiceConfinement($serviceAvailed) {
        $services = explode(', ', $serviceAvailed);
        foreach ($services as $service) {
            if ($service == 'Confinement') {
                return $service;
            }
        }
        ...
        <div class="row">
            ...
            <div class="col-md-4">
                ...
                <select id="categorySelect" class="form-select" aria-label="Default select example">
                    ...
                    <option selected>Confinement</option>
                        ...
                        <?php if(is_array($view)) {?>
                            <?php foreach($view as $view) {?>
                                <tr>
                                    ...
                                    <td><?= getServiceConfinement($view['service_availed']); ?></td>
