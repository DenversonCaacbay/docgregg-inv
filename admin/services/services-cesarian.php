<?php
    ...
    $view = $staffbmis->view_services_cesarian($page, $recordsPerPage);
    $totalRecords = $staffbmis->count_services_cesarian(); // get the total number of records

// Calculate the total number of pages
    $totalPages = ceil($totalRecords / $recordsPerPage);
    function getServiceCesarian($serviceAvailed) {
        $services = explode(', ', $serviceAvailed);
        foreach ($services as $service) {
            if ($service == 'Cesarian Section Surgery') {
                return $service;
            }
        }
        ...
        <div class="row">
            ...
            <div class="col-md-4">
                ...
                <select id="categorySelect" class="form-select" aria-label="Default select example">
                    ...
                    <option selected>Cesarian Section Surgery</option>
                    ...
                    <?php if (is_array($view) && count($view) > 0) { ?>
                            <?php foreach($view as $view) {?>
                                <tr>
                                    ...
                                    <td><?= getServiceCesarian($view['service_availed']); ?></td>
